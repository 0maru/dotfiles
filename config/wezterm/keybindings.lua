...
-- 現在ペインから右方向に等幅分割し、各ペインで同じコマンドを起動する
local function spawn_horizontal_panes(command, count)
  return wezterm.action_callback(function(window, pane)
    local cwd_url = pane:get_current_working_dir()
    ...
    for i = 1, count - 1 do
      local remaining_panes = count - i + 1
      current_pane = current_pane:split {
        direction = 'Right',
        size = (remaining_panes - 1) / remaining_panes,
        cwd = cwd,
...
-- 現在ペインを起点に等分割グリッドを作成する
local function create_grid_layout(columns, rows)
  return wezterm.action_callback(function(window, pane)
    local cwd_url = pane:get_current_working_dir()
    local cwd = cwd_url and cwd_url.path or nil
    ...
    for _, column_pane in ipairs(column_panes) do
...
local keys = {
  ...
  { key = 'Z',     mods = 'LEADER|SHIFT', action = act.TogglePaneZoomState },                             -- ペインのズーム切り替え
  { key = 'r',     mods = 'LEADER',      action = act.ActivateKeyTable { name = 'setting_mode', one_shot = false } }, -- Setting モード
  ...
  { key = '8',     mods = 'LEADER',      action = create_grid_layout(4, 2) },
...
-- === セッティングモード ===
-- LEADER+r で入るモード。Escape/q/CTRL+c で抜ける
local setting_mode = {
  -- ペインリサイズ（1セル単位の細かい調整）
