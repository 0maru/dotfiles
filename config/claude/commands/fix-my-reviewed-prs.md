---
allowed-tools: Bash(git status:*), Bash(git branch:*), Bash(git remote:*), Bash(git fetch:*), Bash(git worktree list:*), Bash(git worktree add:*), Bash(gh auth status:*), Bash(gh api:*), Bash(gh pr list:*), Bash(gh pr view:*), Bash(gh pr checks:*), Bash(gh pr checkout:*), Bash(cd:*), Skill, AskUserQuestion
...
- git worktrees: !`git worktree list 2>/dev/null || echo "(worktree を取得できません)"`
...
7. 選択された PR について既存 worktree を確認し、なければ `git worktree` または git worktree 対応ツールで head branch の worktree を準備する。
8. 各 PR の作業パスと、次に実行する `/review-respond {PR URL}` を出力する。

## 制約

- commit / push / merge / close は行わない。
- worktree 作成には `git worktree` または git worktree 対応ツールを使う。
