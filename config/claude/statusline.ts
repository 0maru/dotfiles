...
const line1 = `${NF_MODEL} ${model} ${SEP}${NF_IN} ${formatTokens(inTokens)}${SEP}${NF_OUT} ${formatTokens(outTokens)}${SEP}${NF_CLOCK} ${latency}s`;
const line2 = parts.join(SEP);
const line3 = `${NF_FOLDER} ${dirDisplay} ${SEP}${gitInfo}`;
