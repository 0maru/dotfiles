...
| `codex` | 別ベンダー（OpenAI）のセカンドオピニオン。Anthropic 系 reviewer の観点バイアスを跨いだ corroboration に使う |

> **常時起動の例外**: `docs/comment only` と `test only` は下記「起動数の目安」のルールが優先する（`hygiene` のみ、または `hygiene` + `test-maintainability` だけ起動。`codex` も同様にスキップ）。SKILL.md の「通常 3〜5 個」は通常 PR の指針であり、例外ケースでは下回ってよい。
> **Codex 起動失敗時の扱い**: `codex` CLI 未インストール / 認証切れ / タイムアウト / リモート PR 対象（ローカル checkout なし）等で起動できない場合は Selected から外し、集約レポートに `Codex: SKIPPED ({理由})` を明記する。Anthropic 系 reviewer のみで集約を続行する。
...
目標は **Anthropic 系 reviewer 3〜5 Agent**（`codex` は別枠）。観点の多様性こそが価値で、同じ観点を増やしても恩恵は薄い。

- 通常 PR: 常時 2 Agent + 条件付き 1〜3 Agent（合計 3〜5） + `codex` 1 枠 = 実体 4〜6 並列。
- 大きい PR / 複数領域横断: Anthropic 系 最大 6 Agent + `codex`。全部起動したくなる場合は、リスクが高い順に絞る。多様性を優先し、近い観点（例: `architecture` と `api-contract` の両方）は片方に絞ることも検討する。
- docs/comment only: `hygiene` のみ（`codex` もスキップ）。**仕様変更を含む docs** は `api-contract` または `architecture` を追加する。仕様変更を含む docs の例: 公開 API の挙動・レスポンス例の追記、必須環境変数・コマンドフラグの追加、互換性のある／ない動作の明記、エンドポイントの追加・削除に言及する README/CHANGELOG。
- test only: `hygiene` と `test-maintainability`（`codex` もスキップ）。本番コードのリスクは原則見ない。

3 を下回る場合は観点の偏りでリスクを見落とす可能性がある。逆に Anthropic 系 6 を超えると Aggregator がノイズに飲まれて Precision が落ちる。`codex` は枠数の上限から除外して数える。
...
**自動チェック失敗で誘発された reviewer は順位リストより優先**: typecheck FAIL → `typescript-type-safety` を必ず Selected に残す、test FAIL → `test-maintainability` を必ず Selected に残す。これらが順位リストの上限（5〜6 個）からはみ出す場合でも切らない（自動チェックが赤い時点で実害が確定しているため）。
