...
> **常時起動の例外**: `docs/comment only` と `test only` は下記「起動数の目安」のルールが優先する（`hygiene` のみ、または `hygiene` + `test-maintainability` だけ起動）。SKILL.md の「通常 3〜5 個」は通常 PR の指針であり、例外ケースでは下回ってよい。
...
目標は **3〜5 Agent**。観点の多様性こそが価値で、同じ観点を増やしても恩恵は薄い。

- 通常 PR: 常時 2 Agent + 条件付き 1〜3 Agent（合計 3〜5）。
- 大きい PR / 複数領域横断: 最大 6 Agent。全部起動したくなる場合は、リスクが高い順に絞る。多様性を優先し、近い観点（例: `architecture` と `api-contract` の両方）は片方に絞ることも検討する。
- docs/comment only: `hygiene` のみ。**仕様変更を含む docs** は `api-contract` または `architecture` を追加する。仕様変更を含む docs の例: 公開 API の挙動・レスポンス例の追記、必須環境変数・コマンドフラグの追加、互換性のある／ない動作の明記、エンドポイントの追加・削除に言及する README/CHANGELOG。
- test only: `hygiene` と `test-maintainability`。本番コードのリスクは原則見ない。

3 を下回る場合は観点の偏りでリスクを見落とす可能性がある。逆に 6 を超えると Aggregator がノイズに飲まれて Precision が落ちる。
